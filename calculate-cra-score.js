...
function calculateTable4Score(data) {
  ...
  // Check if Mauritian - be very flexible with detection
  // Check in extracted nationality field AND in the entire data string
  const isMauritian = natLower.includes('mauritius') || 
                      natLower.includes('mauritian') || 
                      dataLower.includes('nationality:** mauritian') ||
                      dataLower.includes('nationality: mauritian') ||
                      dataLower.includes('nationality:**mauritian') ||
                      dataLower.includes('nationality:\*\* mauritian') ||
                      dataLower.includes('nationality:\*\*mauritian') ||
                      /nationality[:\s*]+mauritian/i.test(data);
  
  console.log('🔍 Mauritian Detection:', {
    nationality,
    natLower,
    isMauritian,
    dataSnippet: data.substring(0, 800),
    hasNationalityKeyword: dataLower.includes('nationality'),
    hasMauritianKeyword: dataLower.includes('mauritian')
  });
  ...
  if (resLower.includes('mauritius') || isMauritian) {
  ...
  return { score, factors, max: 10, isMauritian };
}
...
`;
  ...
  const table1Pct = ((table1.score / table1.max) * 100).toFixed(1);
  const table2Pct = ((table2.score / table2.max) * 100).toFixed(1);
  const table3Pct = ((table3.score / table3.max) * 100).toFixed(1);
  const table4Pct = ((table4.score / table4.max) * 100).toFixed(1);
  const table5Pct = ((table5.score / table5.max) * 100).toFixed(1);
  
  // Build manual verification section if data provided
  let manualVerificationSection = '';
  if (manualVerification) {
    // Format claims history text based on amount
    let claimsText = '';
    if (manualVerification.claimsAmount === 0) {
      claimsText = manualVerification.claimsComments || 'No previous claims';
    } else {
      claimsText = manualVerification.claimsComments || 'Previous claims recorded';
    }
...
• Claims History: MUR ${manualVerification.claimsAmount.toLocaleString()} (${claimsText})
`;
  }
  
  console.log('🔍 CRA Summary - isMauritian:', isMauritian, 'table4:', table4);
  ...
  console.log('🔍 Country Risk Check - isMauritian:', isMauritian, 'Will add country risk?', !isMauritian);
  
  if (!isMauritian) {
    manualChecks.push('• Country Risk: Verify against centralized country risk list');
  } else {
    console.log('✅ Mauritian detected - Country risk check skipped');
  }
...
Dimension Breakdown (Score/Max × Weight = Contribution):
• Nature, Scale & Complexity: ${table1.score}/${table1.max} (${table1Pct}%) × 30% weight → ${breakdown.table1Contribution}
• Products & Services: ${table2.score}/${table2.max} (${table2Pct}%) × 10% weight → ${breakdown.table2Contribution}
• Types of Clients: ${table3.score}/${table3.max} (${table3Pct}%) × 35% weight → ${breakdown.table3Contribution}
• Geography: ${table4.score}/${table4.max} (${table4Pct}%) × 15% weight → ${breakdown.table4Contribution}${isMauritian ? ' ✓ Mauritian (Low Risk)' : ''}
• Delivery Channel: ${table5.score}/${table5.max} (${table5Pct}%) × 10% weight → ${breakdown.table5Contribution}

Weighted Score Calculation:
  ${breakdown.table1Contribution} + ${breakdown.table2Contribution} + ${breakdown.table3Contribution} + ${breakdown.table4Contribution} + ${breakdown.table5Contribution} = ${weightedScore}
